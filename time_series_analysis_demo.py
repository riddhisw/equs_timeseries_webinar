...
def calc_state_depend_H(x_state):
...
def kalman_filter(y_t, dims=1, 
    ...
    gains = np.zeros((num + t_steps_ahead, dims))
    errors = np.zeros((num + t_steps_ahead))
    ...
    if state_dependent_h is True:
        h_matrix = calc_state_depend_H(x_hat[0, :, 0])
    ...
    while (idx_t < num + t_steps_ahead):
        ...
        output  = kalman_step(idx_t, x_hat[idx_t-1, :, 0], P_hat[idx_t-1, :, :], 
...
def kalman_step(t_index, x_hat, P_hat, 
    ...
    residual = calc_residuals(h_matrix, x_hat_apriori, msmt, non_lin_msmt_func)
...
def calc_kalman_gain(h_matrix, P_hat_apriori, R_var):
    ...
    W_gain = np.dot(P_hat_apriori, h_matrix.T)*S_inv
    
    return W_gain.flatten(), S

def calc_residuals(h_matrix, x_hat_apriori, msmt, non_lin_msmt):
    ...
    if non_lin_msmt is False:
        return msmt - np.dot(h_matrix, x_hat_apriori)
    
    if non_lin_msmt is True:
        
        return msmt - calc_non_lin_H(x_hat_apriori) 
...
def estimate_posterior(x_hat_apriori, P_hat_apriori, W_gain, S, residual):
    ...
    '''
    x_hat = x_hat_apriori + W_gain*residual
    P_hat = P_hat_apriori - S*np.outer(W_gain,W_gain.T) 
